...
func (a LDAPAuth) Auth(r *http.Request, usr users.Store, stg *settings.Settings, set *settings.Storage, srv *settings.Server, root string) (user *users.User, err error) {
	var cred jsonCred
	...
	user, err = usr.Get(root, cred.Username)
	if err != nil {
